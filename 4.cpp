...
private:
    int calculateScore(Customer& customer, Sundae& sundae) {
        int score = 0;
        if (equalIgnoreCase(customer.flavor, sundae.flavor)) score++;
        if (equalIgnoreCase(customer.topping, sundae.topping)) score++;
        if (equalIgnoreCase(customer.size, sundae.size)) score++;
        return score;
    }
